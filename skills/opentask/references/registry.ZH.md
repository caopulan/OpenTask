...
对于支持节点级 working memory 的节点类型，orchestrator 应在 run scaffolding 阶段就创建这些文件，即使节点还没开始。此时允许它们是零字节文件或极简占位内容。
只有当这些规范文件都存在时，run scaffolding 才算完成；在此之前，不要派发第一个节点，不要追加 `node.started`，也不要请求 driver review。
